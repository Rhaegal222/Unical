...
class DatoCondiviso:
    ...
    def acquireWriteLock(self):
        ...
        while self.numLettori > 0 or self.ceUnoScrittore:
            dprint(
                f"Il thread {current_thread().name} voleva scrivere, ma trova che ci sono {self.numLettori} lettori e che ceUnoScrittore={self.ceUnoScrittore}. Dunque aspetta.")
            self.condition.wait()
...
class DatoCondivisoSenzaStarvation(DatoCondiviso):
    ...
    def acquireReadLock(self):
        ...
        while self.ceUnoScrittore or \
            ...
            dprint(
                f"Il thread {current_thread().name} trova che {self.numScrittoriInAttesa} scrittori sono in attesa e sono passati {self.numGiriSenzaScrittori} giri senza scrittori. Dunque aspetta")
            self.condition.wait()
        self.numLettori += 1
        #
    ...
    def acquireWriteLock(self):
        ...
        while self.numLettori > 0 or self.ceUnoScrittore:
            dprint(
                f"Il thread {current_thread().name} trova che ci sono {self.numLettori} lettori in attesa e che ceUnoScrittore={self.ceUnoScrittore}. Dunque aspetta")
            self.condition.wait()
        self.ceUnoScrittore = True
        self.numScrittoriInAttesa -= 1
        self.numGiriSenzaScrittori = 0
