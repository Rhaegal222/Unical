...
#
cuochi_e_piatti = { # nome dizionario
    "Cannavacciuolo": ["Pizza", "Pasta", "Tiramisu", "Carbonara"], #a ogni chiave(i cuochi), è associata un'array di valori (piatti)
    "Frankie": ["Hamburger", "Patatine", "Frappe"],
    "Sakura": ["Sushi", "Tempura", "Zuppa di Miso"],
}
#
# Il cuoco svolge il ruolo di produttore (mette elementi nella coda)
#
class Cuoco(Thread):
    ...
    def run(self):
        ...
        while numIterazioni > 0:
            numIterazioni -= 1
            sleep(0.1)
            listaPiattiDiQuestoCuoco = cuochi_e_piatti[self.name] #la variabile per Cannavacciuolo sarà ["Pizza", "Pasta", "Tiramisu", "Carbonara"]
...
if __name__ == "__main__":
    ...
    #
    for c in cuochi_e_piatti: #c itera sui nomi dei cuochi
        newCuoco = Cuoco(q,c)
        newCuoco.start()
    #
