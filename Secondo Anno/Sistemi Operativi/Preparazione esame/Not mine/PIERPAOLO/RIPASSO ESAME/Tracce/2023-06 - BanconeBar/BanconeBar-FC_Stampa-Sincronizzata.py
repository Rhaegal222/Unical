...
class BanconeBar:
    def __init__(self, righe, colonne):
        ...
        self.stampaCondition = Condition(self.lock) # condition per la sincronizzazione
        self.printCounter = 0 # contatore per la stampa
    ...
    def get(self): # estrae un elemento, casualmente, dalla prima fila del bancone (se c'è) e lo restituisce
        with self.lock: 
            while ( (self.printCounter >= 1) or (self.__cisonoElementi() == False) ): # se tutte le colonne del bancone sono vuote oppure se ci sono stampe da fare
                if self.printCounter >= 1: # se ci sono stampe da fare
                    self.stampaCondition.wait() # aspetta che le stampe siano terminate
                else: 
                    self.ceElemento.wait() # aspetta che ci sia almeno un elemento sul bancone
            ...
            self.printCounter += 1 # incrementa il contatore delle stampe da fare
            self.stampaCondition.notify_all() # notifica a tutti che ci sono stampe da fare
    ...
    def put(self, elemento): # inserisce un elemento sul bancone in una colonna casuale tra quelle che hanno meno elementi
        with self.lock: 
            while ( (self.printCounter >=1) or (self.__tuttoPieno()) ):  # se tutte le colonne del bancone sono piene oppure se ci sono stampe da fare
                if self.printCounter >=1:
                    self.stampaCondition.wait() # aspetta che le stampe siano terminate
                else:
                    self.cePostoLibero.wait() # aspetta che ci sia almeno un posto libero sul bancone
    ...
    def print_bancone(self): # metodo per stampare il bancone (viene stampato in modo "orizzontale" e riflesso rispetto alla sua rappresentazione)
        with self.lock: 
            while self.printCounter < 1:
                self.stampaCondition.wait()
            for r in range(self.righe): # 4
                ...
                print()
            self.printCounter = 0 
            self.stampaCondition.notify_all()
